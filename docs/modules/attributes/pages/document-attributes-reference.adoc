...
When set to empty, uses `./favicon.ico` relative to the HTML document.
This should not be set if the head docinfo file already contains a favicon link, as then the generated HTML will contain two favicon links.
|===
